...
def _update_state_cache():
    ...
    _full_resp_cache = _state_cache[:-1] + ',"ip":"' + current_ip + '","wifi":{"ssid":"' + (wifi_config["ssid"] if wifi_config else "") + '"},"mdns":"' + MDNS_HOST + '.local","network":' + _net_cache + ',"wifi_connected":' + wcon + ',"wifi_failures":' + str(wifi_failures) + ',"boot_count":' + str(boot_count) + ',"uptime":' + str(up) + ',"mem_free":' + str(gc.mem_free()) + '}'
