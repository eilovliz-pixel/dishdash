...
def scroll_start(text, count=2, speed=35, callback=None):
    scroll["text"] = text
...
def check_fingerprint():
    ...
    if time.ticks_diff(now, fp_cooldown) < 3000:
        return
    # Only check UART if wakeup pin is HIGH (finger present)
    if FP_WAKE.value() == 1:
        return
    ...
    fp_last_check = now
    turn = state["turn"]
    if turn < len(state["fp"]) and not state["fp"][turn]:
        return
    try:
        fp_uart.read()  # clear buffer
        r = fp_send(bytearray([0x01]))  # GenImg
...
def do_score(player_idx):
    ...
    turn = state["turn"]
    is_turn_player = (player_idx == ((turn - 1) % n))
...
# Quick LED test
led_init()
scroll_start("DISH DASH v4.3", count=1, speed=35)
