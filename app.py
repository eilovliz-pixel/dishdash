...
def check_fingerprint():
    ...
    if time.ticks_diff(now, fp_cooldown) < 1500:
        return
    # Only check UART if wakeup pin is LOW (finger present)
    if FP_WAKE.value() == 1:
        return
    if time.ticks_diff(now, fp_last_check) < FP_CHECK_INTERVAL:
        return
    fp_last_check = now
    print("FP: finger detected, scanning...")
    try:
        fp_uart.read()  # clear buffer
        r = fp_send(bytearray([0x01]))  # GenImg
        rc = fp_code(r)
        if rc != 0:
            print("FP: GenImg failed:", rc, r)
            return
        r = fp_send(bytearray([0x02, 0x01]))  # Img2Tz
        rc = fp_code(r)
        if rc != 0:
            print("FP: Img2Tz failed:", rc, r)
            return
        r = fp_send(bytearray([0x04, 0x01, 0x00, 0x00, 0x00, 0xA3]))  # Search
        rc = fp_code(r)
        if rc != 0 or not r or len(r) < 12:
            print("FP: Search failed:", rc, r)
...
def handle_api(method, path, body):
    ...
    # === FINGERPRINT API ===
    if method == "GET" and path == "/api/fp/diag":
        wake = FP_WAKE.value()
        info = {"wake_pin": wake, "display_active": display_active, "running": state["running"], "turn": state["turn"], "enrolling": fp_enrolling, "ota": bool(scroll.get("_ota"))}
        # Quick sensor test
        try:
            fp_uart.read()
            r = fp_send(bytearray([0x01]))  # GenImg
            info["genimg"] = fp_code(r)
            if fp_code(r) == 0:
                r = fp_send(bytearray([0x02, 0x01]))
                info["img2tz"] = fp_code(r)
                if fp_code(r) == 0:
                    r = fp_send(bytearray([0x04, 0x01, 0x00, 0x00, 0x00, 0xA3]))
                    info["search"] = fp_code(r)
                    if r and len(r) > 11:
                        info["slot"] = r[10] * 256 + r[11]
        except Exception as e:
            info["error"] = str(e)
