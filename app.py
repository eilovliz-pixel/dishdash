...
def handle_api(method, path, body):
    ...
    # === STATE ===
    if path == "/api/state":
        global _state_cache, _state_dirty, _net_cache
        if _state_dirty or _state_cache is None:
            _state_cache = json.dumps(state)
            _net_cache = json.dumps(network_config)
            _state_dirty = False
            gc.collect()
        # Append dynamic fields without creating dict copy
        up = time.ticks_diff(time.ticks_ms(), boot_time) // 1000
        wcon = "true" if (not ap_mode and network.WLAN(network.STA_IF).isconnected()) else "false"
        dynamic = ',"ip":"' + current_ip + '","wifi":{"ssid":"' + (wifi_config["ssid"] if wifi_config else "") + '"},"mdns":"' + MDNS_HOST + '.local","network":' + _net_cache + ',"wifi_connected":' + wcon + ',"wifi_failures":' + str(wifi_failures) + ',"boot_count":' + str(boot_count) + ',"uptime":' + str(up) + '}'
        return _state_cache[:-1] + dynamic
...
def start_server():
    ...
    while True:
        ...
        # Safety reboot every 12h to prevent fragmentation buildup
        if time.ticks_diff(time.ticks_ms(), boot_time) > 43200000:
            print("12h safety reboot")
            save_state()
            machine.reset()
                ...
                for line in hdr.split("\r\n"):
                    if line.lower().startswith("content-length:"):
                        try:
                            ...
                            cl_val = 0
                cl_val = min(cl_val, 16384)  # Safety: max 16KB body
